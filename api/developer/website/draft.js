...
function safeUrl(value, image = false) {
  if (!plainText(value) || /[\\\s]/.test(value) || (typeof value === 'string' && [...value].some((char) => char.charCodeAt(0) < 32))) return false;
  if (!value || (value.startsWith('/') && !value.startsWith('//')) || (!image && value.startsWith('#'))) return true;
  try {
  ...
  return Object.entries(data).every(([key, value]) => {
    ...
    if (key === 'items') {
      ...
      return value.every((item) => {
        if (!isObject(item) || !plainText(item.id) || !item.id.trim() || ids.has(item.id)) return false;
        ids.add(item.id);
        return Object.entries(item).every(([field, entry]) => {
          if (!itemFields[type]?.includes(field)) return false;
          if (field === 'features') return Array.isArray(entry) && entry.length <= 100 && entry.every(plainText);
          return plainText(entry);
        });
      });
    }
    if (key === 'primaryCta' || key === 'secondaryCta') {
      return isObject(value) && Object.keys(value).every((field) => ['label', 'href'].includes(field)) &&
        plainText(value.label) && safeUrl(value.href);
    }
...
function validContent(content) {
  if (!isObject(content) || !safeTree(content) || content.schemaVersion !== 1 ||
      Object.keys(content).some((key) => !['schemaVersion', 'sections'].includes(key)) ||
      !Array.isArray(content.sections) || content.sections.length > 100) return false;
  const ids = new Set();
  return content.sections.every((section) => {
    if (!isObject(section) || !plainText(section.id) || !section.id.trim() || section.id.length > 200 || ids.has(section.id) ||
        typeof section.type !== 'string' || !Object.hasOwn(fields, section.type) || typeof section.enabled !== 'boolean' ||
        !Number.isSafeInteger(section.order) ||
        !(section.variant === null || (plainText(section.variant) && section.variant.length <= 100)) ||
        Object.keys(section).some((key) => !['id', 'type', 'enabled', 'order', 'variant', 'data'].includes(key))) return false;
    ids.add(section.id);
...
function failure(res, error, saving) {
  ...
  const code = known.find((candidate) => error?.code === candidate ||
  ...
  if (saving && (['22023', '23514'].includes(error?.code) || /^INVALID_[A-Z_]+$/.test(String(error?.code || '')) || /\bINVALID_[A-Z_]+\b/.test(String(error?.message || '')))) return res.status(400).json({ ok: false, code: 'INVALID_DRAFT' });
  return res.status(500).json({ ok: false, code: saving ? 'DRAFT_SAVE_FAILED' : 'DRAFT_LOAD_FAILED' });
}
  ...
  try {
    ...
    if (saving) {
      if (String(req.headers['content-type'] || '').split(';')[0].trim().toLowerCase() !== 'application/json') {
        return res.status(400).json({ ok: false, code: 'JSON_REQUIRED' });
      }
      try {
        ...
        body = JSON.parse(raw);
      } catch { return res.status(400).json({ ok: false, code: 'INVALID_PAYLOAD' }); }
    }
    ...
    // Only the RPC changes draft state. Actor identity always comes from the secure server session.
    const { data, error } = await auth.supabaseAdmin.rpc(
      saving ? 'save_website_page_draft' : 'ensure_website_page_draft',
      {
        p_page_id: pageId,
        p_actor_user_id: auth.user.id,
        ...(saving ? { p_content: body.content, p_expected_revision: body.revision } : {}),
      }
    );
    ...
    if (!draft?.id || draft.page_id !== pageId || draft.state !== 'draft' ||
        draft.schema_version !== 1 || !positiveInteger(draft.revision) || !validContent(draft.content)) {
      return failure(res, null, saving);
    }
    return res.status(200).json({ ok: true, draft });
  } catch { return failure(res, null, saving); }
}
